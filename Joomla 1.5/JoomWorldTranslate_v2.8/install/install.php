...
***********************************************************************************************/
// Check to see if a plugin by the same name is already installed
$query = 'SELECT `id` FROM `#__components` WHERE link="option=com_joomworldtranslate"';
$db->setQuery($query);
$componentID = $db->loadResult();

if(!is_null($componentID) && $componentID > 0) {
    $db->setQuery("SELECT * FROM `#__configjoomwtranslate`");
    $onecols = $db->loadResult();
    if(empty($onecols)) {
    $query = "INSERT INTO #__configjoomwtranslate (enable, notranslate, suffix, usingip, ajax_requests, detect_language, worldsjs, notworldstrans, exactranslate, JoomworldtranslatJoomlaVersion, apikey, savewords) VALUES (1, '','1','not_define','com_jcomments','0','','','','','','mysql')";
    $db->setQuery($query);
    }
...
$query = 'SHOW INDEX FROM #__langjoomwtranslate;';
$db->setQuery($query);
$testindex = $db->loadResult();
if(!is_null($testindex) && $testindex > 0) {
    $query = "CREATE INDEX PIndex ON #__langjoomwtranslate ('lang')";
    $db->setQuery($query);
}
...
$rows = 0;
?>
<h2>Joom World Ttranslate Installation</h2>
<table class="adminlist">
	...
	<tbody>
		<tr class="row0">
			<td class="key" colspan="2"><?php echo 'Joom World Ttranslate '.JText::_('Component'); ?></td>
